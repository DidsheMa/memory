...
class StimulusPresentation(QOpenGLWidget):
    ...
    def initialize(self, path, inter_stimulus_interval, mode, n_images=None):
        ...
        indices = []
        indices.extend(range(n_images))
        indices += indices
        shuffle(indices)
        for index in indices:
            self.fields.append((index, False))
        print(self.fields)
        print("initialize finished")

    def show_target(self, stimulus_index, message='', show_matrix=True, skip_wait=False):
    ...
    def paintGL(self):
        ...
        if self.STATUS == PresentationStatus.TARGET_SHOW:  # SHOW TARGET
            ...
            if self.TARGET_INDEX >= 0:
               ...
               for (index, turned) in self.fields:
                   im = self.images[self.imageOrder[index]] if turned else self.images[self.backside]
                   ...
                   self.display_image(im, x_center - (w / 2), y_center - (h / 2), w, h, i == self.TARGET_INDEX)
                   i += 1
               y = self.height()*0.8
               v_align = Qt.AlignTop
            else:
    ...
    def load_images(self):
        ...
        if self.MODE == PresentationMode.SPELLER_MODE:
            self.rows = int(math.floor(math.sqrt(num_im * 2)))
        else:
            self.rows = 1
        self.cols = int(math.ceil(float(num_im * 2)/float(self.rows)))
        self.load_image(self.backside)
